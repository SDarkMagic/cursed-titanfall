...
entity function Grenade_Launch( entity weapon, vector attackPos, vector throwVelocity, bool isPredicted, bool isLagCompensated  )
{
	...
	#if SERVER
	thread Ignite_CookedGrenade( frag, fuseTime )
	#endif
...
#if SERVER
void function Ignite_CookedGrenade( entity grenade, float remainingFuseTime )
{
	if ( !IsValid( grenade ) )
		return
	float baseFuseTime = grenade.GetProjectileWeaponSettingFloat( eWeaponVar.grenade_fuse_time ) //Note that fuse time of 0 means the grenade won't explode on its own, instead it depends on OnProjectileCollision() functions to be defined and explode there. Arguably in this case grenade_fuse_time shouldn't be 0, but an arbitrarily large number instead.
	if ( baseFuseTime <= 0.0 )
		return // Return early since the grenade is most likely being inginted from an OnProjectileCollision callback and we don't want to ignite it early

	grenade.SetDoesExplode( false )
	remainingFuseTime -= 0.5 // Scuffed workaround to nade exploding before being able to ignite it. Just wait  less time than the nade should be active for
	wait remainingFuseTime
	printt( "Post wait", grenade )
	if ( !IsValid( grenade ) )
		return
	if ( grenade.GrenadeHasIgnited() )
		return
	grenade.GrenadeIgnite() //Somehow this is still able to trigger after the grenade explodes. Unsure of what the exact conditions for reproduction are, but one condition seems to be loss of LOS on the nade
}
#endif
