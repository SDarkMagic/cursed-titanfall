...
function FireSniper( entity weapon, WeaponPrimaryAttackParams attackParams, bool playerFired )
{
	...
	if ( chargeLevel > 4 && chargeLevel <= 5 )
	...
	if ( chargeLevel > 5 )
	{
		AddMod( weapon, "max_charge_laser" )
	...
	if ( chargeLevel > 5 )
	{
		ShotgunBlast( weapon, attackParams.pos, attackParams.dir, 1, DF_GIB | DF_BULLET | DF_ELECTRICAL, DF_EXPLOSION | DF_RAGDOLL )
		weapon.EmitWeaponNpcSound( LOUD_WEAPON_AI_SOUND_RADIUS_MP, 0.2 )
		weapon.SetWeaponChargeFractionForced(1.0)
		//RemoveMod( weapon, "max_charge_laser" )
		return 1
		// Add passive mod to gun here that changes the muzzle flash to Ion laser
	}
	else {
...
void function AddMod( entity weapon, string modName )
{
	if ( !weapon.HasMod( modName ) )
	{
		array<string> mods = weapon.GetMods()
